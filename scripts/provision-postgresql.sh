...
provision_postgresql_centos7 () {
    ...
    # install PostgreSQL:
    sudo yum update -y
    sudo yum install -y https://yum.postgresql.org/9.6/redhat/rhel-7-x86_64/pgdg-centos96-9.6-3.noarch.rpm
    sudo yum install -y postgresql96 postgresql96-server postgresql96-contrib postgresql96-devel

    # initialise PostgreSQL database:
    sudo -i -u postgres /usr/pgsql-9.6/bin/pg_ctl initdb

    # allow network connections:
    if ! sudo grep -q "listen_addresses.*${INVENIO_POSTGRESQL_HOST}" \
         /var/lib/pgsql/9.6/data/postgresql.conf; then
        echo "listen_addresses = '${INVENIO_POSTGRESQL_HOST}'" | \
            sudo tee -a /var/lib/pgsql/9.6/data/postgresql.conf
    fi

    # grant access rights:
    if ! sudo grep -q "host.*${INVENIO_POSTGRESQL_DBNAME}.*${INVENIO_POSTGRESQL_DBUSER}" \
         /var/lib/pgsql/9.6/data/pg_hba.conf; then
        echo "host ${INVENIO_POSTGRESQL_DBNAME} ${INVENIO_POSTGRESQL_DBUSER} ${INVENIO_WEB_HOST}/32 md5" | \
            sudo tee -a /var/lib/pgsql/9.6/data/pg_hba.conf
    fi
